...
public class XmlData {
  ...
  {
    StringBuffer rslt = new StringBuffer().append("App Name\tAppGuid\t").append(DeliveryData.getHeader()).append("\t")
        .append(PackageData.getHeader());
    return rslt.toString();
  }

  static public String getCompareHeader()
  {
    StringBuffer rslt = new StringBuffer().append("App Name\tAppGuid\t").append(DeliveryData.getCompareHeader())
        .append("\t").append(PackageData.getCompareHeader());
    return rslt.toString();
  }
  ...
  public String toString()
  {
    ...
    StringBuffer delRslt = new StringBuffer();
    List<DeliveryData> delList = getDeliveries();
    if (delList != null && delList.size() > 0)
    {
      for (DeliveryData delRecord : delList)
      {
        delRslt.delete(0, delRslt.length());
        for (PackageData pkgRec : delRecord.getPackages())
        {
          rslt.append(appRslt).append(delRecord).append(pkgRec).append("\n");
        }
      }
    }
    /**
     * List delList = getDeliveries(); int lastDel = delList.size(); if (lastDel
     * > 0) { DeliveryData delRecord = (DeliveryData) delList.get(lastDel-1);
     * StringBuffer delRslt = new
     * StringBuffer().append(appRslt).append(delRecord).append("\t"); for
     * (PackageData pkgRec: delRecord.getPackages()) {
  ...
  public String compare(String compareType)
  {
    ...
    if (compareFrom >= 0 && compareTo >= 0)
    {
      ...
      StringBuffer fromDelRslt = new StringBuffer().append(appRslt).append(fromDelRecord.getFromRecordString())
          .append("\t");
      StringBuffer toDelRslt = new StringBuffer().append(appRslt).append(toDelRecord.compare(fromDelRecord))
          .append("\t");

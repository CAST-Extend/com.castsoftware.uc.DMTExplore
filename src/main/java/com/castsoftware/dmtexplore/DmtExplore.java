    ...
    try
    {
      ...
      for (Element node : list)
      {
        ...
        if (appName.length() > 0 && appGuid.length() > 0)
        {
          ...
          {
            log.debug(new StringBuffer().append("Application: ").append(appName).append(" found using GUID:")
                .append(appGuid));
        ...
        while (itr.hasNext())
        {
          ...
          for (Element child : items)
          {
            ...
            for (Element reportItem : reportItems)
            {
              ...
              {
                rslt.add(new ScanReportData(reportItem.getAttributeValue("extension", ""),
                    reportItem.getAttributeValue("rootPath", ""),
                ...
                item.setAdded(item.getAdded() + Integer.parseInt(reportItem.getAttributeValue("added", "0")));
                item.setRemoved(item.getRemoved() + Integer.parseInt(reportItem.getAttributeValue("removed", "0")));
                item.setTotal(item.getTotal() + Integer.parseInt(reportItem.getAttributeValue("total", "0")));
              }
            }
          }
        }
...
//      processNetResourceFolder(rootNode, item);
      processSQLCASTUAXExtraction(rootNode, item);
  ...
  private boolean parseValidationDifferenceReport(File base, PackageData item) throws JDOMException, IOException
  {
    ...
    File xmlFile = new File(
        base.getParentFile() + "\\{" + item.getPackageGuid() + "}" + "\\validation_diff_report.pmx");
    if (!xmlFile.exists())
    {
      ...
      if (itr.hasNext())
      {
        ...
        for (int i = 0; i < list.size(); i++)
        {
          ...
          if (node.getName().equalsIgnoreCase("dmtdevmicrosofttechno.NetResourceFilesPackage")
              || node.getName().equalsIgnoreCase("dmtdevnetresourcesextractor.NetResourceFolderExtractor"))
          {
  ...
  private void getCredentials(PackageData item, Element element)
  {
  ...
  private void processSQLCASTUAXExtraction(Element rootNode, PackageData item)
  {
    ElementFilter filter = new ElementFilter("dmtdevsqlcastextractoruaxextraction.SQLCASTUAXExtraction");
    Iterator itr = rootNode.getDescendants(filter);
    while (itr.hasNext())
    {
      item.setPackageType("SQLCASTDatabasePackage");
      Element element = (Element) itr.next();
      item.setHost(element.getAttributeValue("extractorType"));
      item.setRepository(element.getAttributeValue("serverVersionFile", ""));
      item.setRevision(element.getAttributeValue("revision", ""));
    }
      
  }

  private void processNetResourceFolder(Element rootNode, PackageData item)
  {
    ElementFilter filter = new ElementFilter("dmtgitextractor.GITExtractor");
    Iterator itr = rootNode.getDescendants(filter);
    while (itr.hasNext())
    {
      item.setPackageType("NetResourceFolder");
      Element element = (Element) itr.next();
      item.setHost(element.getAttributeValue("rootType"));
      item.setRepository(element.getAttributeValue("folderPath", ""));
      item.setRevision(element.getAttributeValue("revision", ""));
    }
      
  }

  private void processGit(Element rootNode, PackageData item)
  {
    ElementFilter filter = new ElementFilter("dmtgitextractor.GITExtractor");
    Iterator itr = rootNode.getDescendants(filter);
    while (itr.hasNext())
    {
      item.setPackageType("GIT");
      Element element = (Element) itr.next();
      item.setHost(element.getAttributeValue("connectionURL"));
      item.setRepository(element.getAttributeValue("branch", ""));
      item.setRevision(element.getAttributeValue("revision", ""));
      
      getCredentials(item, element);
    }
